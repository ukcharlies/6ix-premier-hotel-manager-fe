...
const CalendarIcon = () => (
  <svg className="h-5 w-5 text-premier-dark" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8">
...
function MonthCalendar({ monthDate, startDate, endDate, onSelectDay }) {
  ...
  const days = useMemo(() => {
    const placeholders = Array.from({ length: offset }).map((_, i) => ({ key: `p-${i}` }));
...
export default function Home() {
  ...
  useEffect(() => {
    function handleClick(e) {
      ...
      if (guestOpen && guestRef.current && !guestRef.current.contains(e.target)) {
        setGuestOpen(false);
      }
    }
  ...
  const guestLabel = `${guests.adults} adult${guests.adults > 1 ? "s" : ""}, ${guests.children} child${
    guests.children !== 1 ? "ren" : ""
  }`;

  const nextMonth = useMemo(() => new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1), [currentMonth]);
  ...
  return (
    <div className="relative min-h-screen bg-premier-dark text-white">
      <div
      ...
      <div className="relative flex flex-col justify-between min-h-screen px-6 sm:px-10 lg:px-16 py-12">
        <div className="max-w-4xl mt-10 sm:mt-14">
          <p className="text-premier-light/90 text-lg mb-4">6ix Premier Collection</p>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight text-white">
            Discover Your Perfect Luxury Escape
          </h1>
          <p className="mt-6 text-lg sm:text-xl text-premier-light/90 max-w-2xl">
            Signature stays, bespoke experiences, and seamless check-ins crafted for modern travelers.
          </p>
                  ...
                  <div className="absolute z-20 mt-3 w-full max-w-3xl lg:max-w-2xl bg-white rounded-2xl shadow-2xl border border-premier-gray p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      <div>
                        ...
                        <MonthCalendar monthDate={currentMonth} startDate={startDate} endDate={endDate} onSelectDay={handleSelectDay} />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-premier-dark mb-2">
                          {nextMonth.toLocaleDateString("en-US", { month: "long", year: "numeric" })}
                        </p>
                        <MonthCalendar monthDate={nextMonth} startDate={startDate} endDate={endDate} onSelectDay={handleSelectDay} />
                      </div>
                    </div>
                      ...
                      {["adults", "children"].map((type) => (
                        <div key={type} className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="w-8 text-center text-premier-dark font-semibold">{guests[type]}</span>
                            <button
                              type="button"
                              onClick={() => adjustGuests(type, 1)}
                              className="h-10 w-10 rounded-xl border border-premier-gray text-premier-dark font-bold hover:border-premier-copper transition-colors"
